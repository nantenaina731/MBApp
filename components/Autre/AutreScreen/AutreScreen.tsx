...
export default function ParolesScreen() {
  ...
  const data: OtherSong = JSON.parse(song as string);
  ...
  useEffect(() => {
    SecureStore.getItemAsync('favoris').then((stored) => {
      if (stored) {
        const favoris: OtherSong[] = JSON.parse(stored);
  ...
  const handleFavorite = async () => {
    ...
    try {
      ...
      const favoris: OtherSong[] = stored ? JSON.parse(stored) : [];
  ...
  <TouchableOpacity onPress={handleFavorite} style={styles.iconBtn1}>
    <Ionicons
